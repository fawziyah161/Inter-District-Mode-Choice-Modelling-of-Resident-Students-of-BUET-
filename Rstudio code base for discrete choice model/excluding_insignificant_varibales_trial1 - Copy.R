...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate") {
  ...
  V[["ac_bus"]] = ASC_ac_bus + B_traveltime_acbus * (tt_acbus) +B_gender_acbus * gender  + B_carowner* car_own+
                  B_incomef_acbus * (income_fam/ fam_mem_no) + B_distance_acbus * (distance_km /100)+B_comfort_acbus * comfort_level + B_cost_acbus * (tc_acbus/100) 
  ...
  V[["train"]] = ASC_train  + B_traveltime_train *  (tt_train) +B_gender_train * gender + B_incomes_train * income_self - B_lowincome_train *(1/ income_fam)+B_carowner* car_own +
                   + B_distance_train * (distance_km/100)+ B_cost_train * (tc_train/100)
  
  
  V[["personal_car"]] = ASC_personal_car  +B_gender_carp * gender + B_incomef_carp * (income_fam/ fam_mem_no) +B_comfort_carp * comfort_level +  B_cost_carp * (tc_carpersonal_n/100)+
                       B_comf_inc * (comfort_level * income_fam)
    
  
  V[["rental_car"]] = ASC_rental_car+B_gender_carr * gender+  B_incomes_carr * income_self+ B_carowner* car_own+
                    + B_distance_carr * (distance_km /100)+B_comfort_carr * comfort_level  + B_cost_carr * (tc_carrental_n/100)
