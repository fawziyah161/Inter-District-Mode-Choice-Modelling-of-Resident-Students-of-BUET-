...
### Set core controls
apollo_control = list(
  modelName       = "ASC sing cost coeff all time coeff",
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate") {
  ...
  V[["bus"]] = ASC_bus + B_cost * (tc_bus/100) + B_traveltime_bus * tt_bus
  
  V[["ac_bus"]] = ASC_ac_bus +B_cost * (tc_acbus/100) + B_traveltime_acbus * tt_acbus
  
  V[["train"]] = ASC_train +B_cost * (tc_train/100) + B_traveltime_train * tt_train
  
  V[["personal_car"]] = ASC_personal_car + B_cost * (tc_carpersonal_n / 100) + B_traveltime_carp * tt_car_personal 
  
  V[["rental_car"]] = ASC_rental_car + B_cost * (tc_carrental_n / 100) + B_traveltime_carr * tt_car_rental
...
### Set core controls
apollo_control = list(
  modelName       = "ASC all cost coeff one time coeff",
  modelDescr      = "ASC +travel cost+travel time",
  indivID         = "ID", # Ensure your dataset includes an individual ID column
  outputDirectory = "output_Aumy"
)
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate") {
  ...
  V[["bus"]] = ASC_bus + B_cost_bus * (tc_bus/100) + B_traveltime * tt_bus
  
  V[["ac_bus"]] = ASC_ac_bus +B_cost_acbus * (tc_acbus/100) + B_traveltime * tt_acbus
  
  V[["train"]] = ASC_train +B_cost_train * (tc_train/100) + B_traveltime * tt_train
  
  V[["personal_car"]] = ASC_personal_car + B_cost_carp * (tc_carpersonal_n / 100) + B_traveltime * tt_car_personal 
  
  V[["rental_car"]] = ASC_rental_car + B_cost_carr * (tc_carrental_n / 100) + B_traveltime * tt_car_rental
